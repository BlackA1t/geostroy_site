...
export function parseAdminRequestDetailsInput(body: unknown): AdminRequestDetailsInput {
  ...
  if (!serviceType || !description || !name || !phone) {
    throw new Error("Заполните тип услуги, описание, имя и телефон.");
  }
...
export function getAdminRequestDetailsChanges(
  current: EditableRequestDetails,
  next: AdminRequestDetailsInput
) {
...
export function buildAdminRequestEditComment(
  changes: ReturnType<typeof getAdminRequestDetailsChanges>
) {
